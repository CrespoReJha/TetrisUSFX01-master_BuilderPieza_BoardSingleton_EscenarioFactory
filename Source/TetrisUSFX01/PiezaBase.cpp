...
void APiezaBase::BeginPlay()
{
	Super::BeginPlay();
    //SpawnBlocks();
    //GEngine->AddOnScreenDebugMessage(-1, 30, FColor::Green, TEXT("El tipo color es: ") + FString::FromInt(getTipoColor()));
}

// Called every frame
void APiezaBase::Tick(float DeltaTime)
{
	Super::Tick(DeltaTime);

}

void APiezaBase::SpawnBlocks()
{
...
/*int APiezaBase::GenerarColor(int _tipoColor)
{
    int Color;
    if (_tipoColor == 1) {
        Color = FMath::RandRange(0, 6);
    }
    return Color;
}*/

void APiezaBase::EndPlay(const EEndPlayReason::Type EndPlayReason)
{
...
void APiezaBase::DrawDebugLines()
{
    for (ABlock* B : Blocks)
    {
...
void APiezaBase::Dismiss()
{
    Blocks.Empty();
}

void APiezaBase::MoveLeft()
{
...
bool APiezaBase::MoveDown(bool PlaySound)
{
...
bool APiezaBase::CheckWillCollision(std::function<FVector(FVector OldLocation)> ChangeBeforeCheck)
{
